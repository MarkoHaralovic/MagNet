...
class DeepGlobe(BaseDataset):
    ...
    def inverse_frequency_weighting(self, alpha = 0.5):
        ...
        total_pixels = 0
        for i in tqdm(range(len(self.files)), desc="Updating weights of classes"):
            _, label,_,_ = self.__getitem__(i)
            classes,frequencies = np.unique(label, return_counts=True)
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image = cv2.imread(os.path.join(self.root, "deepglobe", item["img"]), cv2.IMREAD_COLOR)
        size = image.shape

        label = cv2.imread(os.path.join(self.root, "deepglobe", item["label"]), cv2.IMREAD_COLOR)
    ...
    def multi_scale_inference(self, config, model, image, scales=[1], flip=False):
        ...
        for scale in scales:
            ...
            if scale <= 1.0:
                ...
                rows = np.int_(np.ceil(1.0 * (new_h - self.crop_size[0]) / stride_h)) + 1
                cols = np.int_(np.ceil(1.0 * (new_w - self.crop_size[1]) / stride_w)) + 1
